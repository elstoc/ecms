    ...
    describe('getMetadata', () => {
        describe('throws an error', () => {
            ...
            it('if no content directory is found', async () => {
                ...
                await expect(component.getMetadata()).rejects.toThrow(
                    new NotFoundError(
                        'A content directory does not exist for the path my-component',
                    ),
                );
        ...
        describe('returns metadata', () => {
            ...
            it('gets metadata by parsing the component file on the first run', async () => {
                ...
                expect(mockStorage.getContentFileModifiedTime).toHaveBeenCalledWith(
                    'my-component.yaml',
                );
    ...
    describe('getGallery', () => {
        describe('throws an error', () => {
            it('if no yaml file is found', async () => {
                ...
                await expect(component.getGallery('x')).rejects.toThrow(
                    new NotFoundError('A yaml file does not exist for the path my-component'),
                );
                expect(mockStorage.contentFileExists).toHaveBeenCalledWith('my-component.yaml');
            });
            ...
            it.each(['markdown', 'videodb'])(
                'when called for a %s component',
                async (type: string) => {
                    mockStorage.contentFileExists.mockReturnValue(true);
                    mockStorage.contentDirectoryExists.mockReturnValue(true);
                    mockStorage.getContentFileModifiedTime.mockReturnValue(1234);
                    ...
                    await expect(component.getGallery('x')).rejects.toThrow(
                        new NotFoundError('No gallery component found at the path my-component'),
                    );
                },
            );
        });
    ...
    describe('getMarkdown', () => {
        describe('throws an error', () => {
            it('if no yaml file is found', async () => {
                ...
                await expect(component.getMarkdown('x')).rejects.toThrow(
                    new NotFoundError('A yaml file does not exist for the path my-component'),
                );
                expect(mockStorage.contentFileExists).toHaveBeenCalledWith('my-component.yaml');
            });
            ...
            it.each(['gallery', 'videodb'])(
                ...
                async (type: string) => {
                    ...
                    await expect(component.getMarkdown('x')).rejects.toThrow(
                        new NotFoundError('No markdown component found at the path my-component'),
                    );
                },
            );
        });
    ...
    describe('getVideoDb', () => {
        describe('throws an error', () => {
            it('if no yaml file is found', async () => {
                ...
                await expect(component.getVideoDb('x')).rejects.toThrow(
                    new NotFoundError('A yaml file does not exist for the path my-component'),
                );
                expect(mockStorage.contentFileExists).toHaveBeenCalledWith('my-component.yaml');
            });
            ...
            it.each(['markdown', 'gallery'])(
                ...
                async (type: string) => {
                    ...
                    await expect(component.getVideoDb('x')).rejects.toThrow(
                        new NotFoundError('No videodb component found at the path my-component'),
                    );
                },
            );
        });
