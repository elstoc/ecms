  ...
  describe('throws OASParsingError when', () => {
    ...
    describe('a parameter definition', () => {
      it('contains an empty record', async () => {
        ...
        await expect(oasParser.parseOAS()).rejects.toThrow(
          new OASParsingError('invalid path/query parameters for endpoint delete:/some/path'),
        );
      });
    ...
    describe.each(['query', 'path'])('a %s parameter definition', (inParam) => {
      ...
      it('has a repeated name', async () => {
        ...
        await expect(oasParser.parseOAS()).rejects.toThrow(
          new OASParsingError(`duplicate ${inParam} parameter foo in endpoint put:/some/path`),
        );
      });
      ...
      it('has an object schema', async () => {
        const dereferencedSchema = {
          paths: {
            '/some/path': {
              put: {
                parameters: [{ in: inParam, name: 'some-name', schema: { type: 'object' } }],
              },
            },
          },
        };
        ...
        const dereferencedSchema = {
          paths: {
            '/some/{somename}/path/{somename}': {
              put: {
                parameters: [{ in: 'path', name: 'somename', schema: { type: 'string' } }],
              },
            },
          },
        };
      ...
      it('exists in the endpoint path but not in the OAS parameter list', async () => {
        const dereferencedSchema = {
          paths: {
            '/some/path/{somename}': {
              put: {
                parameters: [{ in: 'query', name: 'somename', schema: { type: 'string' } }],
              },
            },
          },
        };
      ...
      it('has an array-type schema', async () => {
        const dereferencedSchema = {
          paths: {
            '/some/path': {
              put: {
                parameters: [{ in: 'path', name: 'somename', schema: { type: 'array' } }],
              },
            },
          },
        };
    ...
    describe('an object schema (e.g. requestBody)', () => {
      it.each([
        ...
        await expect(oasParser.parseOAS()).rejects.toThrow(
          new OASParsingError('object requestBody at endpoint put:/some/path has no properties'),
        );
      });
      ...
      it.each([
        ...
        await expect(oasParser.parseOAS()).rejects.toThrow(
          new OASParsingError('property requestBody.foo at endpoint put:/some/path has no schema'),
        );
      });
  ...
  describe('builds a valid validation schema', () => {
    ...
    it('creates a complete set of validation objects where requestBody, query and path params are present', async () => {
      ...
      const { requestBodySchema, requestBodyRequired, queryParamsSchema, pathParamsSchema } = (
        await oasParser.parseOAS()
      )['put:/some/path/{field3}/{field4}'];
