...
export class OASParser {
    ...
    public async parseOAS(): Promise<{ [endpoint: string]: EndpointValidationSchemas }> {
        ...
        for (const [path, oasPathDetails] of Object.entries(oasPaths)) {
            ...
            for (const [endpointMethod, oasEndpointDetails] of Object.entries(
                oasPathDetailsRecord,
            )) {
    ...
    private parseOASForEndpoint(
        ...
        if (oasEndpointDetails.parameters || pathParamsFromEndpoint.length > 0) {
            ...
            validationParams = {
                ...validationParams,
                ...this.parseOASParameters(
                    oasEndpointDetails.parameters,
                    endpoint,
                    pathParamsFromEndpoint,
                ),
            };
        }
    ...
    private parseOASRequestBody(
        ...
        validationParams.requestBodySchema = this.parseOASObject(
            bodySchema,
            endpoint,
            'requestBody',
        );
    ...
    private parseOASObject(
        ...
        try {
            ...
            if (!additionalProperties) {
                throw new OASParsingError(
                    `object ${fullPath} at endpoint ${endpoint} has no properties`,
                );
            }
        }
    ...
    private parseOASInteger(
        ...
        if (oasIntSchema.minimum !== undefined) {
            if (
                typeof oasIntSchema.minimum !== 'number' ||
                !Number.isInteger(oasIntSchema.minimum)
            ) {
                throw new OASParsingError(
        ...
        if (oasIntSchema.maximum !== undefined) {
            if (
                typeof oasIntSchema.maximum !== 'number' ||
                !Number.isInteger(oasIntSchema.maximum)
            ) {
                throw new OASParsingError(
